...
package D_Bus.Types.Basic_Generic is
   ...
   private
      ...
      function "+" (X : Inner) return Outer
      is ((I => Padded_Inners.Padded_Type (X)));
   ...
   private
      ...
      overriding
      function Signature (X : Outer) return Single_Signature
      is (Fixed_Types.Outer (X).Signature);

      overriding
      function Image (X : Outer) return String
      is (Fixed_Types.Outer (X).Image);

      use type Fixed_Types.Outer;
      function "+" (X : Inner) return Outer
      is (Fixed_Types."+" (X) with null record);
      function "+" (X : Outer) return Inner
      is (+Fixed_Types.Outer (X));
   ...
   private
      package Padded_Data_Length_Types is new
        Padded_Types (Data_Length_Type, Data_Length_Type'Size / 8);
