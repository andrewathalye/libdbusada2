...
package body D_Bus.Messages is
   ...
   -- Validity Checking --
   -----------------------
   Interface_Regexp : constant GNAT.Regexp.Regexp := GNAT.Regexp.Compile
   ...
   -- Valid_Member --
   ------------------
   Member_Regexp : constant GNAT.Regexp.Regexp := GNAT.Regexp.Compile
     (Pattern => "^([a-zA-Z0-9_])*$",
      Glob => True,
      Case_Sensitive => True);
   ...
   -- Valid_Bus --
   ---------------
   Bus_Regexp : constant GNAT.Regexp.Regexp := GNAT.Regexp.Compile
     (Pattern =>
   ...
   is
      ...
      Fields : Field_Maps.Map;
   begin
      if Reply_To.Serial = Invalid_Serial
         or Reply_To.Flags.No_Reply_Expected
      then
   ...
   begin
      M.Arguments.Splice
        (Before => D_Bus.Types.Argument_Lists.No_Element,
         Source => Arguments);
   end Add_Arguments;
   ...
   function Is_Reply (Original, Reply : Message) return Boolean is
      use type D_Message_Serial;
   begin
      return Original.Flags.No_Reply_Expected
         and then Original.Serial = +D_Message_Serial
           (Reply.Fields (F_Reply_Serial).Get);
   end Is_Reply;
...
--     (System.High_Order_First, System.Low_Order_First);
   ...
   package Field_Dicts is new D_Bus.Types.Containers.Dicts
     (Key_Type_Code => 'y',
      Value_Signature => "v");
   ...
   begin
      ...
      Item := (Serial => Valid_Message_Serial'(+RMH.Serial),
               others => <>);
      Item.M_Type := RMH.M_Type;
      Item.Flags := RMH.Flags;

      for Cursor in RMH.Fields.Iterate_KCT loop
         Item.Fields.Insert
           (+D_Field_Type (Key (Cursor)),
      ...
      begin
         for Signature of Types loop
            Item.Arguments.Append
              (D_Bus.Types.Dispatching_Read (Stream, Signature.To_String));
         end loop;
      end;
   end Read;
   ...
   begin
      ...
      RMH.Body_Length := +Interfaces.Unsigned_32
        (D_Bus.Types.Size (Item.Arguments));
      ...
      for Cursor in Item.Fields.Iterate loop
         RMH.Fields.Insert
           (+Field_Maps.Key (Cursor),
            Field_Maps.Element (Cursor));
      end loop;
